...
public class ModbusTcpServer implements ModbusServer {
    ...
    private final AtomicReference<ModbusServices> services = new AtomicReference<>(new ModbusServices() {});

    public ModbusTcpServer(ModbusServerConfig config, ModbusTcpServerTransport transport, ModbusServices services) {
    ...
    protected ModbusTcpFrame handleModbusTcpFrame(ModbusTcpFrame frame, ModbusTcpRequestContext context)
            throws Exception {
        ...
        try {
            ...
            ModbusResponsePdu response =
                    switch (functionCode) {
                        ...
                        default -> throw new ModbusResponseException(
                                requestPdu.getFunctionCode(), ExceptionCode.ILLEGAL_FUNCTION.getCode());
                    };
            ...
            ByteBuffer pdu =
                    ByteBuffer.allocate(2).put((byte) fc).put((byte) ec).flip();
     ...
     */
    public static ModbusTcpServer create(ModbusTcpServerTransport transport, ModbusServices modbusServices) {
