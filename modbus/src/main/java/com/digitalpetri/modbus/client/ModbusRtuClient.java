...
public class ModbusRtuClient extends ModbusClient {
        ...
        if (promise != null) {
            ...
            if (functionCode < 0x80) {
                if (functionCode != promise.functionCode) {
                    ...
                    promise.future.completeExceptionally(new ModbusException(
                            "function code mismatch: %s != %s".formatted(promise.functionCode, functionCode)));
                    ...
                    pending.forEach(p -> p.future.completeExceptionally(new ModbusException("synchronization error")));
                } else {
                    try {
    ...
    private record ResponsePromise(int slaveId, int functionCode, CompletableFuture<ModbusResponsePdu> future) {}
}
