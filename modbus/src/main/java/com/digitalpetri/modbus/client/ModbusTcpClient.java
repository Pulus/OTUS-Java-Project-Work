...
public class ModbusTcpClient extends ModbusClient {
    ...
    public CompletionStage<ModbusResponsePdu> sendAsync(int unitId, ModbusRequestPdu request) {
        ...
        return cs.thenApply(buffer -> {
            try {
                ModbusPdu decoded = config.responseSerializer().decode(request.getFunctionCode(), buffer);
    ...
    private CompletionStage<ByteBuffer> sendBufferAsync(int unitId, ByteBuffer buffer) {
        ...
        TimeoutHandle timeout = config.timeoutScheduler()
                .newTimeout(
                        t -> {
        ...
        transport.send(new ModbusTcpFrame(header, buffer)).whenComplete((v, ex) -> {
            if (ex != null) {
                ResponsePromise promise = promises.remove(header.transactionId());
                if (promise != null) {
    ...
    private void onFrameReceived(ModbusTcpFrame frame) {
        ...
        if (promise != null) {
            ...
            if (functionCode == promise.functionCode) {
                ...
                promise.future.completeExceptionally(new ModbusResponseException(promise.functionCode, exceptionCode));
            } else {
                promise.future.completeExceptionally(
    ...
    /**
     * The promise of some future response PDU bytes and the function code of the originating
     * request.
     *
     ...
     * @param timeout a {@link TimeoutHandle} handle to be cancelled when the response is
     *     received.
     */
    private record ResponsePromise(int functionCode, CompletableFuture<ByteBuffer> future, TimeoutHandle timeout) {}
