...
class ReadWriteMultipleRegistersRequestTest {
    ...
    void serializer() {
        ...
        var request =
                new ReadWriteMultipleRegistersRequest(address, quantity, writeAddress, writeQuantity, writeValues);

        ReadWriteMultipleRegistersRequest.Serializer.encode(request, buffer);

        buffer.flip();

        ReadWriteMultipleRegistersRequest decoded = ReadWriteMultipleRegistersRequest.Serializer.decode(buffer);
