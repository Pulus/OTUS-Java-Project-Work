    ...
    void readCoils() throws Exception {
        ...
        while (remaining > 0) {
            ReadCoilsResponse response =
                    services.readCoils(new TestModbusRequestContext(), 0, new ReadCoilsRequest(address, quantity));
    ...
    void readHoldingRegisters() throws Exception {
        ...
        while (remaining > 0) {
            ReadHoldingRegistersResponse response = services.readHoldingRegisters(
                    new TestModbusRequestContext(), 0, new ReadHoldingRegistersRequest(address, quantity));
